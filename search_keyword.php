...
$sql = "SELECT JokeID, Joke_question, Joke_answer, jokes_table.users_user_id, user_name FROM Jokes_table JOIN 
users ON users.user_id = Jokes_table.users_user_id WHERE Joke_question LIKE '%$keywordfromform%'";
echo $sql;
...
$stmt = $mysqli->prepare("SELECT JokeID, Joke_question, Joke_answer, jokes_table.users_user_id, user_name 
FROM jokes_table JOIN users ON jokes_table.users_user_id = users.user_id WHERE Joke_question LIKE ?");
$stmt->bind_param("s", $keywordfromform);
$stmt->execute();
$stmt->store_result();
